...
#include <steam/steam_api.h> // Ensure this is included for SteamFriends()

GameOverState::GameOverState(CubeGame* game) : State(game) {
    // Center "Game Over" title
    game->GetHUD().addElement("gameOver", "Game Over", 36, sf::Vector2f(SCREEN_WIDTH / 2.f - 100.f, 150.f), 
...
void GameOverState::Render() {
    ...
    // Draw players and enemies (frozen state)
...
void GameOverState::UpdateLeaderboard() {
    ...
    for (const auto& player : game->GetPlayers()) {
        ...
        if (!steamName || steamName[0] == '\0') {
            steamName = "Unknown"; // Fallback if Steam API fails or name is empty
        }
