      ...
      menuVisible(false)
{
    if (game->IsHost() && !game->IsGameStarted()) {
        ...
        game->GetNetworkManager()->SyncEnemiesFull();  // Initial full sync
    }
...
void GameplayState::UpdatePlayingState(float dt) {
    ...
    if (localPlayer.isAlive) {  // Only allow movement and shooting if alive
    ...
    game->GetEntityManager()->checkCollisions(
        [&](const Bullet& b, uint64_t enemyId) {
            ...
            int damage = 10; // Default damage value, adjust as needed
            char hitBuffer[128]; // Increased size to accommodate more fields
...
void GameplayState::RenderEnemies() {
    for (auto& enemyPair : game->GetEnemies()) {
...
void GameplayState::ShootBullet() {
    if (game->GetShootCooldown() > 0 || !game->GetLocalPlayer().isAlive) {
        return;
    }

    const Player& localPlayer = game->GetLocalPlayer();
    float startX = localPlayer.x + 10.f;
    float startY = localPlayer.y + 10.f;
    ...
    uint64_t shooterSteamID = localPlayer.steamID.ConvertToUint64();
...
void GameplayState::NextLevel() {
    ...
    if (game->IsHost()) {
        game->GetNetworkManager()->SyncEnemiesFull();  // Full sync at level start
    }
}
